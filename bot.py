...
async def ask(prompt):
    ...
    return await loop.run_in_executor(executor, _claude_sync, prompt)
...
async def cmd_chats(update, ctx):
    ...
    if not chats:
        await update.message.reply_text("No chats tracked yet. Add me to a group and I will auto-track it.")
        return
...
async def cmd_summary(update, ctx):
    ...
    ans = await ask(f"You are a BD assistant for Soneium. Chat: {db.get_chat_name(chat_id)}.\n\n{ctx_txt}\n\nGive a comprehensive BD status summary.")
...
async def _do_query(update, q):
    ...
    if not results:
        await t.edit_text("No messages found. Send some messages in your tracked group chats first.")
        return
    ctx_txt = "\n".join(f"[{m['chat_name']} | {m['timestamp'][:16]}] {m['username']}: {m['text']}" for m in results)
...
def _kbd(chats, sel):
    ...
    rows += [
        [InlineKeyboardButton("Select All", callback_data="bc_all"),
