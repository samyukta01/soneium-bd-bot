...
class Database:
    ...
    def _init(self):
        with self._conn() as conn:
            with conn.cursor() as c:
                ...
                c.execute("CREATE INDEX IF NOT EXISTS idx_msg_text ON messages USING gin(to_tsvector('english', COALESCE(text,'')))")

    def permit_chat(self, chat_id, chat_name):
        with self._conn() as conn:
            with conn.cursor() as c:
                c.execute("INSERT INTO permitted_chats(chat_id,chat_name) VALUES(%s,%s) ON CONFLICT(chat_id) DO UPDATE SET chat_name=EXCLUDED.chat_name",
                          (chat_id, chat_name))
    ...
    def get_messages(self, chat_id, limit=300):
        with self._conn() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as c:
                c.execute("SELECT * FROM messages WHERE chat_id=%s ORDER BY timestamp DESC LIMIT %s",
                          (chat_id, limit))
    ...
    def search_messages_across_chats(self, query, limit=300):
        ...
        with self._conn() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as c:
                c.execute(
                    f"SELECT m.* FROM messages m JOIN permitted_chats p ON m.chat_id=p.chat_id WHERE {conditions} ORDER BY m.timestamp DESC LIMIT %s",
                    params)
                return [dict(r) for r in reversed(c.fetchall())]
